...
- **Import:** runs inside a single transaction against a *staging* set of tables (or an entirely separate temp SQLite file), and only swaps it in as the live database after full validation succeeds. If anything fails mid-import, the transaction rolls back and the pre-existing database is untouched — the safety backup is a second line of defense, not the primary one.
