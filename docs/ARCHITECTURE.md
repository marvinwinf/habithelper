...
- The overall app streak has the same problem in miniature: whether a fully-elapsed day "breaks" `app_streak_cache` depends on whether any routine was actually completed that day, which can only be known after that day's routines have been reconciled. Rather than advancing `app_streak_cache` incrementally from its `reconciled_through_date` watermark, the streak is **fully re-derived from the whole event log** every time it can change — a direct completion, an undo, a retroactive calendar completion, and the startup reconciliation pass all call the same `recomputeAppStreakCacheTx` (`src/services/routineService.ts`), which replays every non-deleted routine's events day by day (`computeAppStreak` in `src/domain/streaks/appStreak.ts`) from the earliest recorded occurrence up to and including today. This is what makes a **backdated completion** correctly lift the global streak: an incremental "+1 on the first completion of a new day" update can only ever see the newest day and silently ignores a past gap filled in via the calendar, so a routine streak built retroactively would climb while the global streak stood still. A full replay counts each day exactly as the events dictate. `today` itself only ever extends the streak (it is still in progress and must never reset just because its routines are still pending); every earlier day applies the reset-on-fully-missed rule. `app_streak_cache.reconciled_through_date` is retained as bookkeeping (set to yesterday on each recompute) but is no longer a resume point, since the whole log is replayed each time. The exact reset behavior on a fully-missed day is an open product question — see `docs/IMPLEMENTATION_PLAN.md`'s Open Product Questions.
