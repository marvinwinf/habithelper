...
- Category-tinted cards carry a soft light sheen (`CardSheen`): the `surface` tone falls in from the top-left and fades out along the diagonal, so the tint reads as gently lit paper instead of a flat color block. The sheen adds light, never a second hue — one dominant color per card is unchanged — and because it only ever lightens the tint it can only raise text contrast (pinned by the contrast test).
